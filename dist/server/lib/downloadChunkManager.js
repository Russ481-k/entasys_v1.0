...
export class DownloadChunkManager {
  constructor(downloadId, searchParams) {
    this.chunks = new Map();
  ...
  async downloadChunk(config) {
    ...
    try {
      ...
      while (hits.length > 0 && processedRows < chunk.totalRows) {
        ...
        if (processedRows >= chunk.totalRows) break;
        const scrollResponse = await client.request({
          path: '/_search/scroll',
          method: 'POST',
          body: {
            scroll: '5m',
  ...
  getCurrentProgress() {
    ...
    const fileName =
      (firstChunk === null || firstChunk === void 0
        ? void 0
        : firstChunk.fileName) ||
  ...
  emitFileReady(fileName, clientFileName) {
    console.log(
...
export class DownloadChunkManagerInstance {
  ...
  createManager(downloadId, searchParams) {
