...
interface CleanupStrategy {
  name: string;
  description: string;
  execute: (indices: IndexInfo[]) => Promise<string[]>;
}
...
class IndexMonitor {
  ...
  async getAllIndices(): Promise<IndexInfo[]> {
    try {
      return await this.opensearch.request<IndexInfo[]>({
  ...
  private createOldestFirstStrategy(): CleanupStrategy {
    return {
      name: 'oldest_first',
      description: '가장 오래된 인덱스부터 삭제',
      execute: async (indices: IndexInfo[]): Promise<string[]> => {
  ...
  private createSmallestFirstStrategy(): CleanupStrategy {
    return {
      name: 'smallest_first',
  ...
  // 정리 전략 3: 도메인별 균등 삭제
  private createBalancedDomainStrategy(): CleanupStrategy {
    return {
      name: 'balanced_domain',
      description: '각 도메인에서 균등하게 오래된 인덱스 삭제',
      execute: async (indices: IndexInfo[]): Promise<string[]> => {
        // 도메인별로 그룹화
        const domainGroups: { [key: string]: IndexInfo[] } = {};
  ...
  async getIndexStatistics(): Promise<void> {
    const allIndices = await this.getAllIndices();
    const userIndices = allIndices.filter(
      (index) =>
        !index.index.startsWith('.') &&
